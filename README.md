# 知识卡片与错题复习管理系统 v1.4.4

基于 C++17 控制台的学习管理系统，围绕“录入知识 / 错题 -> 复习调度 -> 自测练习 -> 统计反馈 -> 数据维护”形成完整学习闭环。

v1.4.4 在 v1.4.3 隐藏底层主键热修的基础上，继续统一术语：用户在界面中操作的连续数字统一称为“序号”；底层持久化字段只在“核心数据模型”章节展开说明，其他文档和界面使用“底层主键”“关联字段”等描述。
...
| 终端显示 | 使用 ANSI True Color、窗口式边框、四阶段小人欢迎页、主菜单工作台、子菜单入口面板、内部页面标题、小节标题和统一提示样式增强演示效果 |
...
├── README.md
├── compile_flags.txt
├── terminal_sprites_demo/   # v1.4.0 欢迎页小人视觉素材与生成样例
├── data/
│   ├── users.txt
│   ├── cards.txt
│   ├── wrongs.txt
...
│   │   └── v1.4/              # v1.4.0~v1.4.4 CLI 界面增强迭代报告
│   └── Project Proposal/
│       ├── 知识卡片与错题复习管理系统方案.md
│       ├── v1.0版本总方案.md
│       ├── v1.0计划.md
...
└── src/
    ├── main.cpp              # 程序入口、一级菜单、登录后主菜单、衰减扫描与主菜单视图数据组装
    ...
    ├── tui.h / tui.cpp       # v1.4.x 轻量终端界面渲染：欢迎页、主菜单工作台、页面标题、小节和统一提示
    ├── date_utils.h / date_utils.cpp
    │                         # 统一日期解析、校验、日期差和日期加减
    ├── storage.h / storage.cpp
    │                         # data 目录初始化、文本文件读写、自增键生成
...
说明：v1.4.0 已将面向菜单展示的轻量终端界面渲染拆入 `tui.h / tui.cpp`，v1.4.1 继续把主要子菜单入口接入该表现层，v1.4.2 进一步覆盖模块内部标题、结果小节、业务提示和存储错误提示，v1.4.3 修复交互界面暴露底层主键的问题，v1.4.4 统一“序号 / 底层主键 / 关联字段”术语；`utils.h / utils.cpp` 保留 ANSI 初始化、清屏、暂停、输入解析和日期兼容包装。方案与计划文档位于 `docs/Project Proposal/`，开发日志与迭代报告位于 `docs/Development Log/`。

---

## 7. 核心数据模型

本节是当前 README 中唯一展开底层持久化字段名的位置。这里的 `userId`、`cardId`、`wrongId`、`linkedCardId`、`logId`、`itemId` 都属于内部 ID 或内部关联字段，不作为普通交互界面的用户操作入口。
...
`linkedCardId` 用于记录“错题转知识卡片”后生成卡片的内部 ID，默认值为 `-1`。
...
卡片和错题列表对用户显示连续序号 `[1] [2] [3]`，用户按序号操作。底层真实主键仍保留在文件中，但交互界面不显示底层主键，也不要求用户输入底层主键。
...
- 错题转知识卡片：一键将错题生成为新卡片，并通过内部关联字段防重复转换
- 卡片按学科/章节/标签筛选，按创建时间/下次复习时间/掌握度排序
- 数据维护模块 `maintenance`：查看已逻辑删除的记录并恢复
- UI 表现层序号映射：用户操作 `[1] [2] [3]` 序号，不再直接接触底层主键
...
不改变业务能力和数据格式，专注提升控制台演示效果与主菜单信息密度：

- 新增 `tui` 表现层：`main.cpp` 负责准备视图数据，`tui.cpp` 负责渲染欢迎页、主菜单工作台、统一提示和输入提示
- 欢迎页升级为 Claude Code inspired 终端窗口风格，使用三色窗口按钮、ANSI True Color、边框面板和 `C / R / S / L` 四个小人表达 `CARD -> REVIEW -> STUDY -> LOOP`
- 登录后主菜单升级为工作台视图，集中展示当前日期、数据目录、今日待复习数量、有效卡片数、有效错题数和薄弱点推荐
- 原有菜单序号、`getline` 输入方式、命令行参数、存储格式和自动化测试入口保持不变
- v1.4.1 继续统一卡片、错题、今日复习、自测练习、统计分析和数据维护六个子菜单入口的标题、分组菜单、输入提示和非法输入错误样式
- v1.4.2 继续统一用户表单、卡片 / 错题内部 CRUD 与查询页面、复习会话、自测练习、统计结果、维护操作和存储写入错误提示
- v1.4.3 紧急修复底层主键泄露：创建成功、错题转卡、复习列表、复习历史、错题详情和维护检查不再显示真实底层主键
- v1.4.4 统一“序号”术语，用户可操作数字一律称为序号，内部字段名收束到核心数据模型章节

---

## 14. 当前验收标准

当前版本达到以下条件即可认为 v1.4.4 序号与底层字段术语治理完成：
...
- 登录后会触发记忆衰减扫描。
- 主菜单能显示薄弱知识点推荐。
- 欢迎页能显示窗口式边框、四阶段小人和 `CARD -> REVIEW -> STUDY -> LOOP` 闭环提示。
- 登录后主菜单能显示工作台式状态摘要，包括今日待复习数量、有效卡片数、有效错题数、数据目录和薄弱点推荐。
- 卡片、错题、今日复习、自测练习、统计分析和数据维护入口子菜单能显示统一的窗口式面板、菜单分组、输入提示和非法输入错误提示。
- 用户登录 / 注册 / 改密、卡片和错题 CRUD、查询结果、复习流程、自测练习、统计结果和维护操作页面能显示统一页面标题、小节标题和状态通知。
- 存储写入失败不再输出裸 `[错误]` 文本，而是走统一 TUI 错误提示。
- 创建知识卡片成功后不显示底层主键。
- 错题转知识卡片成功或重复转换拦截时不显示底层主键或内部关联值。
- 今日复习列表、复习历史和不可用任务提示不显示底层定位值。
- 错题详情中的关联卡片只显示“无 / 已关联”，不显示底层关联值。
- 数据一致性检查面向交互用户显示记录位置和问题原因，不显示底层卡片定位值。
- 用户可输入和可选择的数字统一称为“序号”，不再使用旧称谓。
- 工作区真实 `data/cards.txt` 与 `data/wrongs.txt` 不应因为本轮验证残留业务数据修改。
- 工作区不应残留本轮临时构建产物。
- 可以进行随机自测和薄弱点专项练习。
- 可以查看统计分析。
...
- v1.4.x 欢迎页、主菜单和子菜单入口使用 Unicode 边框和 ANSI True Color；若终端字体或颜色支持较弱，视觉效果会降级，但数字菜单流程不受影响。
...
1. 进入 v1.4.5：继续打磨列表详情页、表格式结果展示和 CLI 非交互输出策略。
2. 继续评估数据备份 / 导入 / 导出能力。
