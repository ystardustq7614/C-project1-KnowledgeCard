# 知识卡片与错题复习管理系统 v1.3.7

基于 C++17 控制台的学习管理系统，围绕“录入知识 / 错题 -> 复习调度 -> 自测练习 -> 统计反馈 -> 数据维护”形成完整学习闭环。

v1.3.7 在 v1.3.6 源码注释规范落地基础上，将同一注释标准扩展到测试模块，补齐测试入口、E2E helper、业务分支脚本和 C++ 单元测试的职责、约束、断言口径和数据隔离说明。
...
当前版本达到以下条件即可认为 v1.3.7 测试注释规范迭代完成：
...
1. 进入 v1.4.0：汇总 v1.3.1 ~ v1.3.7，形成稳定闭环版本。
2. 后续再评估数据备份 / 导入 / 导出能力。
