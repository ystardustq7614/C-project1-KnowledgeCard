# 知识卡片与错题复习管理系统 v1.4.0

基于 C++17 控制台的学习管理系统，围绕“录入知识 / 错题 -> 复习调度 -> 自测练习 -> 统计反馈 -> 数据维护”形成完整学习闭环。

v1.4.0 在 v1.3.7 源码与测试注释基础上，按 `教学代码注释要求.md` 补齐项目级导读、阅读路径、概念映射、输入输出变化、易错点和可实验修改点，让这个分支更适合作为新人学习 C++ 多文件项目的入口。
...
项目目标：
- 用一个本地控制台系统演示“数据建模 -> 文本持久化 -> 菜单交互 -> 复习算法 -> 自动化测试”的完整闭环。

本项目适合谁：
- 已经学过 C++ 基础语法，但还不熟悉多文件项目、状态流和测试脚本的新同学。

你将学到：
- 如何把 `User`、`Card`、`WrongQuestion`、`ReviewLog` 拆成稳定的数据模型。
- 如何让控制台输入经过业务模块，最终保存到 `data/*.txt`。
- 如何用简化 SM-2、记忆衰减和薄弱点推荐把学习记录变成复习任务。
- 如何通过 `--data-dir` 和 PowerShell 脚本做不污染真实数据的回归测试。

建议阅读顺序：
1. 先读 `README.md` 的“核心数据模型”“持久化规则”“复习与推荐算法”。
2. 再读 `src/models.h`、`src/globals.h`，确认所有模块共享的数据结构。
3. 接着读 `src/main.cpp`，建立启动、登录、菜单分发和衰减扫描的总流程。
4. 按业务顺序读 `storage`、`user`、`card`、`wrong`、`review`、`practice`、`stats`、`maintenance`。
5. 最后读 `tests/` 和 `tools/`，理解项目如何验证行为。
...
关键概念与代码映射：
- 数据模型 -> `src/models.h`
- 全局运行状态 -> `src/globals.h`
- 文本持久化 -> `src/storage.cpp`
- 菜单入口与主流程 -> `src/main.cpp`
- 知识卡片管理 -> `src/card.cpp`
- 错题管理与错题转卡片 -> `src/wrong.cpp`
- 今日复习闭环 -> `src/review.cpp` + `src/algo_sm2.cpp`
- 登录后的记忆衰减 -> `src/main.cpp` + `src/algo_decay.cpp`
- 薄弱点推荐与专项练习 -> `src/algo_recommend.cpp` + `src/practice.cpp`
- 数据一致性检查 -> `src/maintenance.cpp`
- 自动化验证 -> `tools/run_all_tests.ps1` + `tests/`

第一次阅读建议重点关注：
- `userId` 如何让多用户数据隔离贯穿所有业务模块。
- 展示序号如何映射到底层 `cardId / wrongId`，避免用户直接操作全局编号。
- `active=false` 为什么表示逻辑删除，而不是立即移除文件记录。
- `review.cpp` 如何把卡片和错题统一为 `ReviewTask`。

首轮阅读可暂时跳过：
- ANSI 颜色显示细节。
- PowerShell 测试脚本中的路径保护 helper。
- 历史版本计划文档。

[实验] 建议从这三个小改动开始观察现象：
1. 修改 `src/algo_sm2.cpp` 中“记牢”后的间隔阶梯，再运行算法测试观察断言变化。
2. 修改 `src/algo_decay.cpp` 的 3 天宽限期，再用测试 fixture 观察衰减触发时机。
3. 在 `src/storage.cpp` 暂时去掉 `|` 的转义，再运行 E2E 测试看数据文件如何被分隔符破坏。
...
│   │   ├── v1.3/              # v1.3.0~v1.3.7 迭代报告
│   │   └── v1.4/              # v1.4.0 教学化注释迭代报告
│   └── Project Proposal/
│       ├── 知识卡片与错题复习管理系统方案.md
│       ├── v1.0版本总方案.md
│       ├── v1.0计划.md
...
面向新人学习分支，保留业务行为不变，重点增强“第一次阅读代码”的理解路径：

- README 新增项目级教学导读，明确阅读顺序、主流程、模块映射、首轮可跳过内容和可实验修改点。
- 核心源码注释升级为标签化说明，覆盖 `[导读]`、`[输入输出]`、`[学习重点]`、`[易错点]`、`[简化说明]` 和 `[实验]`。
- 算法、存储、日期、复习、维护等关键路径补充输入输出变化、业务边界和误改风险。
- 测试与工具入口版本标记同步到 v1.4.0，方便新人把运行输出与当前文档对应起来。

---

## 14. 当前验收标准

当前版本达到以下条件即可认为 v1.4.0 教学化注释迭代完成：
...
- README 提供项目目标、阅读顺序、一次运行主流程、模块映射和可实验修改点。
- 核心源码使用统一教学标签说明位置、输入输出、易错点、简化边界和实验入口。
...
1. 继续教学化重构：把重复的列表序号映射逻辑收敛成更易讲解的公共 helper。
2. 后续再评估数据备份 / 导入 / 导出能力。
