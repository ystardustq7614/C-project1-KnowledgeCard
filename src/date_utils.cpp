...
/*
[导读]
- 本文件是项目统一日期工具，业务层不要各自手写日期差或日期加减。

[输入输出]
- 输入：YYYY-MM-DD 字符串。
- 输出：解析后的 Date、日期差天数、加减后的 YYYY-MM-DD 字符串或合法性判断。

[学习重点]
- 合法公历日期会转换为“自 1970-01-01 起的天数”再计算，覆盖跨月、跨年和闰年。

[易错点]
- 非法日期的日期差返回 0，避免衰减算法误扣分；坏数据修复交给 maintenance.cpp。
- addDaysToDate 遇到非法日期会原样返回，方便一致性检查暴露原始问题。

[实验]
- 在 tests/test_date_utils.cpp 增加一个跨闰年用例，观察 daysBetweenDates 和 addDaysToDate 是否互相一致。
*/
